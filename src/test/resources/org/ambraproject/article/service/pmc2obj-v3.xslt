...
<xsl:stylesheet version="2.0"
  ...
  <xsl:template name="main-dc">
    ...
    <!-- 1/25/12: updated for 3.0 -->
    <xsl:if test="$meta/permissions">
      <rights>
        <xsl:call-template name="xml-to-str"><xsl:with-param name="xml" select="$meta/permissions/copyright-holder"/></xsl:call-template>
        <xsl:text>. </xsl:text>
        <xsl:call-template name="xml-to-str"><xsl:with-param name="xml" select="$meta/permissions/license/license-p"/></xsl:call-template>
      </rights>
    </xsl:if>

    <!-- 1/25/12: updated for 3.0 -->
    <conformsTo>&nlmpub;<xsl:value-of select="$article/@dtd-version"/>/journalpublishing3.dtd</conformsTo>
    <xsl:if test="$meta/permissions/copyright-year">
      <copyrightYear><xsl:value-of select="$meta/permissions/copyright-year"/></copyrightYear>
    </xsl:if>
    ...
    <bibliographicCitation>
      ...
      <xsl:call-template name="gen-citation">
        ...
        <xsl:with-param name="journal"
             ...
             else $jnl-meta/journal-title-group/journal-title"/>
  ...
  <xsl:template name="gen-ref">
    <xsl:for-each select="$article/back/ref-list/ref">
      <references>
        <xsl:variable name="refCit" select="element-citation | mixed-citation"/>
        <xsl:call-template name="gen-citation">
          <xsl:with-param name="type"
              select="if ($refCit/@publication-type) then my:map-pub-type($refCit/@publication-type)
                      else ()"/>
          <xsl:with-param name="key"      select="label"/>
          <xsl:with-param name="year"     select="my:find-int($refCit/year[1], 4)"/>
          <xsl:with-param name="dispYear" select="$refCit/year[1]"/>
          <xsl:with-param name="month"    select="$refCit/month[1]"/>
          <xsl:with-param name="day"      select="$refCit/day[1]"/>
          <xsl:with-param name="volume"   select="$refCit/volume[1]"/>
          <xsl:with-param name="volNum"   select="my:find-int($refCit/volume[1], 1)"/>
          <xsl:with-param name="issue"    select="$refCit/issue[1]"/>
          <xsl:with-param name="title"
              select="if ($refCit/article-title) then $refCit/article-title[1]
                      else if ($refCit/source)   then $refCit/source[1]
                      else ()"/>
          <xsl:with-param name="pub-loc"  select="$refCit/publisher-loc[1]"/>
          <xsl:with-param name="pub-name" select="$refCit/publisher-name[1]"/>
          <xsl:with-param name="pages"
              select="if ($refCit/page-range) then $refCit/page-range[1]
                      else if ($refCit/lpage) then concat($refCit/fpage, '-', $refCit/lpage)
                      else $refCit/fpage"/>
          <xsl:with-param name="elocation-id" select="$refCit/elocation-id | $refCit/fpage"/>
          <xsl:with-param name="journal"
              select="if ($refCit/@publication-type = 'journal')
                        then $refCit/source[1] else ()"/>
          <xsl:with-param name="note"     select="$refCit/comment[1]"/>
          <xsl:with-param name="editors"
              select="if ($refCit/person-group[@person-group-type = 'editor'])
  ...
  <xsl:template name="gen-citation">
    ...
    <!-- 2/13/12: updated for 3.0 -->
    <xsl:for-each select="$collab-authors">
      <collaborativeAuthors><xsl:apply-templates select="."/></collaborativeAuthors>
    </xsl:for-each>
  ...
  <xsl:function name="my:format-contrib-name" as="xs:string?">
    ...
    <xsl:choose>
      ...
      <xsl:when test="$contrib/collab">
        <xsl:apply-templates select="$contrib/collab"/>
  ...
  <!-- 1/25/12: updated for 3.0 -->
  <xsl:function name="my:map-pub-type" as="xs:anyURI">
    <xsl:param name="pub-type" as="xs:string"/>
    <xsl:variable name="uri" as="xs:string" select="
      if      ($pub-type = 'book')       then '&bibtex;Book'
      else if ($pub-type = 'journal')    then '&bibtex;Article'
      else if ($pub-type = 'other')      then '&bibtex;Misc'
      else '&bibtex;Misc'
      "/>
