...
public interface SyndicationService {
   ...
   *
   * @param articleDoi the doi of the article to query
   * @param target     the syndication target
   * @return the matching syndication, if it exists, else null
   */
  public Syndication getSyndication(String articleDoi, String target);
   ...
   * <code>articleDoi</code> and <code>syndicationTarget</code>, then no action will be performed.
   ...
   * progress</i> and a <code>lastModified</code> within the past number of days defined by the configuration
   * property <code>ambra.virtualJournals.JOURN AL_KEY.syndications.display.numDaysInPast</code>, where
   * <i>JOURNAL_KEY</i> is the <code>journalKey</code> parameter.  By default, a <i>failed</i> or <i>in progress</i>
   * Syndication can be up to 30 days old and still appear in this list.
   *
   ...
   *
   * @param articleDoi        The ID for the Article which will be syndicated to the <code>syndicationTarget</code>
