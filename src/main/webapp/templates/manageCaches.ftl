      ...
      <fieldset>
        ...
        <p>
          <#list cacheKeys as cacheKey>
            <@s.url id="remove" namespace="/" action="manageCaches"
              cacheAction="remove" cacheName="${cacheName}" cacheKey="${cacheKey}"/>
            <@s.url id="get" namespace="/" action="manageCaches"
    ...
    <table border="1" cellpadding="2" cellspacing="0">
      <#list cacheStats.keySet().toArray() as cacheName>
        <tr>
          <th>${cacheName}</th>
          <#if cacheName != "">
            <@s.url id="clearStatistics" namespace="/" action="manageCaches"
              cacheAction="clearStatistics" cacheName="${cacheName}" />
            <@s.url id="removeAll" namespace="/" action="manageCaches"
